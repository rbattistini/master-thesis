...
I would like to thank my supervisor Giovanni Ciatto and co-supervisor Gianluca Aguzzi, for their guidance and support throughout this thesis.
%
...
%
I am grateful for the opportunity to collaborate with them on a paper, based on the thesis idea presented here, that is currently under submission to the ECAI 2025 conference.
...
\end{lstlisting}
%
This approach borrows \jason{}'s extended \agentspeak{} syntax~\cite{BordiniHW2007} for failure handling plans to express a plan (namely, $p_{genex}$) reacting to the failure event provoked by the absence of a plan for goal \inlineAsl{G}, to which the agent should react by generating a plan for it, and finally pursue goal \inlineAsl{G} one more time.
%
...
%
\Cref{lst:format-instructions} shows an example of this approach, used in the PGP prototype, to instruct the LLM to format its responses with a hybrid YAML and Prolog syntax.
...
%
\Cref{lst:nl-hints} shows an example of application to document an admissible goal, using a syntax that will be detailed in~\cref{sec:generative-agent-spec}.
...
%
\Cref{lst:system-prompt} shows the system prompt sent in all the \ac{PGP} invocations and~\cref{lst:user-prompt} shows the user prompt with both hints and remarks included.

\lstinputlisting[
    float=htbp,
    label={lst:system-prompt},
    caption={The system prompt used by the robot explorer application.},
    basicstyle=\scriptsize\ttfamily,
    captionpos=b,
    mathescape=false
]{listings/prompt-system.md}

\lstinputlisting[
    float=htbp,
    label={lst:user-prompt},
    caption={The user prompt used by the robot explorer application.},
    basicstyle=\scriptsize\ttfamily,
    captionpos=b,
    mathescape=false
]{listings/prompt-user.md}
...
\lstinputlisting[
    float=htbp,
    label={lst:claude-response},
    caption={Example response given by Claude Sonnet 4 (beautiful-volhard). This is one of the few successful PGP invocations which use a prompt with no hints or remarks.},
    basicstyle=\scriptsize\ttfamily,
    captionpos=b,
    mathescape=false
]{listings/generation_result_beautiful_volhard.txt}


\lstinputlisting[
    float=htbp,
    label={lst:deepseek-response},
    caption={Example response given by Deepseek V3 Chat (stoic-rhodes).},
    basicstyle=\scriptsize\ttfamily,
    captionpos=b,
    mathescape=false
]{listings/generation_result_stoic_rhodes.txt}

\lstinputlisting[
    float=htbp,
    label={lst:gpt-response},
    caption={Example response given by GPT 4.1 (stupefied-lewin).},
    basicstyle=\scriptsize\ttfamily,
    captionpos=b,
    mathescape=false
]{listings/generation_result_stupefied_lewin.txt}

\lstinputlisting[
    float=htbp,
    label={lst:gemini-response},
    caption={Example response by Gemini 2.5 Flash (frosty-kare). This is the response that got the highest score by the LLM judge, which coincidentally was Gemini 2.5 Flash itself, and was one of the few answers the model provided that were successful.},
    basicstyle=\scriptsize\ttfamily,
    captionpos=b,
    mathescape=false
]{listings/generation_result_frosty_kare.txt}
...
\Cref{tab:byPromptAndModel}, \cref{fig:prompt-stats} and~\cref{fig:prompt-stats-cont} present an evaluation of the defined metrics for each of the chosen models under three different prompting conditions.
%
...
\Cref{tab:byTempAndModel}, \cref{fig:temp-stats} and~\cref{fig:temp-stats-cont} show how the value of the metrics change as the temperature varies.
%
...
\Cref{tab:byTempPromptAndModel} provides an overview of the best performing combinations of parameters considering both the temperature and the prompt type.
%
